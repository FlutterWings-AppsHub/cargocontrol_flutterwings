...
final inViajesInprogessNotiController = ChangeNotifierProvider((ref) {
  final api = ref.watch(inViajesApisProvider);
  return InViajesInprogessNotiController(datasource: api);
});

class InViajesInprogessNotiController extends ChangeNotifier {
  final InViajesApisImplements _datasource;
  InViajesInprogessNotiController({
  ...
  Future getInprogessViajes({required WidgetRef ref,required String industryId,required String searchWord}) async {
    if (searchWord != '') {
  ...
  Future firstTime({required WidgetRef ref,required String industryId}) async {
    ...
    QuerySnapshot querySnapshot =
        await _datasource.getInprogressViajes(limit: limit, snapshot: _lastSnapshot,vesselId: vesselId, industryId: industryId);
