...
final inViajesCompletedNotiController = ChangeNotifierProvider((ref) {
  final api = ref.watch(inViajesApisProvider);
  return InViajesCompletedNotiController(datasource: api);
});

class InViajesCompletedNotiController extends ChangeNotifier {
  final InViajesApisImplements _datasource;
  InViajesCompletedNotiController({
  ...
  Future firstTime({required WidgetRef ref,required String industryId}) async {
    ...
    QuerySnapshot querySnapshot =
        await _datasource.getCompletedViajes(limit: limit, snapshot: _lastSnapshot, vesselId: vesselId, industryId: industryId);
